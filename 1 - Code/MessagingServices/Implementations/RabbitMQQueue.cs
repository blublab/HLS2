    ...
    internal class RabbitMQQueue<T> : IQueueServices<T>, IDisposable where T : DTOType<T>
    {
        ...
        public void Send(T data)
        {
            ...
            try
            {
                ...
                this.channel.BasicPublish("", Queue, null, body);
            }
            catch (Exception ex)
            {
        ...
        public Task<T> ReceiveAsync(Func<T, MessageAckBehavior> function)
        {
            ...
            try
            {
                ...
                {
                    BasicDeliverEventArgs ea = (BasicDeliverEventArgs)this.consumer.Queue.Dequeue();
                    byte[] body = ea.Body;
                    ...
                    if (msgAckBehavior == MessageAckBehavior.AcknowledgeMessage)
                    {
                        ...
                        Log.Debug(string.Format("Message '{0}' was acknowledged in queue '{1}'.", message, this.Queue));
                    }
                    else
                    {
                        this.channel.BasicNack(ea.DeliveryTag, false, true);
                        Log.Debug(string.Format("Message '{0}' was left in queue '{1}'.", message, this.Queue));
                    }
